...
## Чтобы работало:
* сначала надо запустить `create.py`, чтобы создались матрицы и векторы, которые будут потом умножаться с запросом, инициализировались векторайзеры, модели и т.д.  
* потом `app.py`
